...
<template>
  <section id="inicio" ref="root" class="hero-sequence" :class="{'static-film':reducedMotion}">
    <div ref="film" class="hero cinematic-hero" :class="`chapter-${chapter}`">
      ...
      <div class="container hero-content"><div class="eyebrow"><span class="status-dot"></span> TU CIUDAD TIENE ALGO QUE DECIRTE.</div><div class="film-title-stage"><Transition name="film-title" mode="out-in"><div :key="chapter"><span class="film-scene-label">0{{ chapter+1 }} / {{ chapters[chapter] }}</span><h1 v-if="chapter===0">Tu ciudad,<br><em>10 minutos</em><br>antes.</h1><h1 v-else-if="chapter===1">Cada ruta<br>tiene una<br><em>historia.</em></h1><h1 v-else>Tu mañana.<br>Ahora con<br><em>tiempo.</em></h1><p class="hero-description">{{ chapter===0?'El camión va hacia tu calle.':chapter===1?'Ubicación, movimiento y conexión.':'Un aviso. Una cosa menos de qué preocuparte.' }}<br><b>{{ chapter===0?'Esta vez, tú ya lo sabes.':chapter===1?'La ciudad empieza a hablar.':'El servicio llega. Tú ya estás listo.' }}</b></p></div></Transition></div><div class="hero-buttons"><a class="btn btn-primary" href="#experiencia">Ver cómo funciona <ArrowUpRight :size="20"/></a><a class="hero-secondary" href="#ecosistema">Descubrir el ecosistema <ArrowDown :size="16"/></a></div><p class="hero-caption">Así es como tu ciudad <span>se conecta.</span></p></div>
