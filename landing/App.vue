...
const perspectives = [
  {icon:Truck,role:'OPERADOR',title:'Un toque, toda la ciudad conectada.',copy:'GPS, red móvil y servidor confirman en segundos que la unidad y la ruta están en marcha.'},
  {icon:UserRound,role:'CIUDADANO',title:'Un aviso antes de que llegue.',copy:'RutaLimpia calcula cuándo se acerca el camión para que tengas tiempo de sacar tus residuos.'},
  {icon:Building2,role:'MUNICIPIO',title:'El panorama completo de la flota.',copy:'Cobertura, incidencias y avance de cada ruta, centralizados en un solo lugar.'},
];
...
<template>
  <div class="landing" :class="{'motion-paused':paused,'reduce-motion':reducedMotion}">
    ...
    <main id="contenido">
      ...
      <section id="experiencia" class="experience-section section"><div class="container"><div class="section-heading reveal"><div><span class="section-index">02 — TRES PERSPECTIVAS, UNA RUTA</span><h2>Una ruta.<br><span>Tres formas de vivirla.</span></h2></div><p>El operador la conduce, el ciudadano la espera y el municipio la supervisa. Así se conecta cada parte.</p></div><div class="impact-grid"><article v-for="(item,i) in perspectives" :key="item.role" class="reveal"><div><component :is="item.icon" :size="24"/><span>0{{ i+1 }}</span></div><small>{{ item.role }}</small><h3>{{ item.title }}</h3><p>{{ item.copy }}</p></article></div></div></section>
