...
// Nada reactivo por cada cuadro de scroll: la barra se mueve directo y sólo cambia el estado al cruzar el umbral.
function onScroll(){ if(scrollFrame)return;scrollFrame=requestAnimationFrame(()=>{const total=document.documentElement.scrollHeight-window.innerHeight;const p=total>0?window.scrollY/total:0;if(progressBar.value)progressBar.value.style.transform=`scaleX(${p})`;const show=p>.13;if(show!==showAccess.value)showAccess.value=show;scrollFrame=0;}); }
...
<template>
  <div class="landing" :class="{'motion-paused':paused,'reduce-motion':reducedMotion}">
    ...
    <header class="site-header"><div class="nav-container"><a class="brand" href="#inicio" aria-label="RutaLimpia, inicio"><img class="brand-logo" :src="logoUrl" alt="" width="47" height="44">ruta<span>limpia</span><i></i></a><nav :class="{open:menuOpen}" aria-label="Navegación principal"><a href="#experiencia" @click="menuOpen=false">La experiencia</a><a href="#ecosistema" @click="menuOpen=false">Ecosistema</a><a href="#municipio" @click="menuOpen=false">Para tu municipio</a></nav><div class="header-actions"><a class="header-access" href="acceso.html" aria-label="Entrar a la plataforma">Entrar a la plataforma <ArrowUpRight :size="16"/></a><button class="menu-toggle icon-btn" :aria-expanded="menuOpen" aria-label="Abrir o cerrar menú" @click="menuOpen=!menuOpen"><X v-if="menuOpen" :size="21"/><Menu v-else :size="21"/></button></div></div><div ref="progressBar" class="reading-progress" style="transform:scaleX(0)"></div></header>
    <main id="contenido">
      ...
      <section id="experiencia" class="experience-section section"><div class="container"><div class="section-heading reveal"><div><span class="section-index">02 — NO LO IMAGINES. ACTÍVALO.</span><h2>Una ruta.<br><span>Tres formas de vivirla.</span></h2></div><p>Siéntate al volante, recibe el aviso y mira la ciudad desde el municipio. Todo empieza contigo.</p></div><DemoExperience :technical="technical" :reduced-motion="reducedMotion" :paused="paused"/><div class="under-demo"><span>Tu siguiente paso puede ser real.</span><a class="inline-link" href="acceso.html">Ir a la plataforma <ArrowUpRight :size="16"/></a></div></div></section>
      ...
      <section class="final-section section container"><div class="final-route" aria-hidden="true"><span></span><Truck :size="30"/><span></span></div><span class="section-index">UN CAMIÓN. UNA RUTA. UN CIUDADANO AVISADO A TIEMPO.</span><h2 class="reveal">De recolectar residuos<br>a <em>conectar ciudades.</em></h2><p>No te explicamos RutaLimpia. Te dejamos probarlo.<br>Ahora, entra al ecosistema.</p><div class="final-actions"><a class="btn btn-primary" href="acceso.html">Probar el ecosistema <ArrowUpRight :size="19"/></a><a class="btn btn-outline" href="#municipio">Implementar en mi municipio</a></div><a class="inline-link" href="#arquitectura" @click="architectureOpen=true">Conocer la tecnología <ArrowRight :size="15"/></a></section>
    </main>
    <footer class="site-footer container"><a class="brand" href="#inicio"><img class="brand-logo" :src="logoUrl" alt="" width="47" height="44">ruta<span>limpia</span><i></i></a><p>Tu ciudad, 10 minutos antes.<small>© {{ new Date().getFullYear() }} RutaLimpia · Michoacán, México</small></p><a href="acceso.html">Acceso a la plataforma <ArrowUpRight :size="16"/></a></footer>
    <a v-if="showAccess" class="floating-access" href="acceso.html"><span>Entrar a RutaLimpia</span><ArrowUpRight :size="18"/></a>
  </div>
</template>
